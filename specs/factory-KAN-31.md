...
- The prompt now states the grouping as an exact greedy procedure: flush the current small round before each 250-or-more-line singleton, flush it when the next small file would make the total exceed 300, append the final partial round, and require the flattened rounds to equal the changed-file inventory. This makes the exact-once and boundary behavior explicit without adding a second scheduler.
...
- Updated `.github/workflows/ai-review.yml` to replace per-file review instructions with an exact deterministic size-aware greedy procedure: files at 250+ physical lines are singleton rounds, while smaller files remain in changed-file order and are grouped only through an inclusive 300-line limit. The prompt includes the 249/250, 300/301, and ordered grouping boundary examples.
- Extended `factory/src/tests/ai-review-workflow.test.ts` with static coverage for the thresholds, full-file line-count requirement, exact-once round partition, one-subagent-per-round synthesis, the unchanged publisher lifecycle, and removal of the old per-file scheduling instructions.
- Existing workflow publication and label behavior was not modified. Final validation passed with `node --test dist/tests/ai-review-workflow.test.js` (2 tests), `npm.cmd test` (125 tests), and the final `npm.cmd run build` TypeScript check.
