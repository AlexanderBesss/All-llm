...
export function buildSpecContent({ issue, runId, branchName, generatedAt = new Date().toISOString() }) {
  ...
  return [
    ...
    "",
    "> This specification is generated for an unattended factory run. It is the scope and decision record for one parent Jira issue, one lead implementation agent with optional investigation sub-agents, one independent reviewer, one branch, and one pull request.",
    "",
    "## Metadata",
    "",
    "| Field | Value |",
