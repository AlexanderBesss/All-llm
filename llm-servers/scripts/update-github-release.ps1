<#
.SYNOPSIS
    Shared module for updating GitHub release binaries.
.DESCRIPTION
...
function Get-CurrentVersion {
    ...
    if ($Marker) { return $Marker.Name.Substring('VERSION-'.Length) }
    return $null
}
...
function Update-GitHubRelease {
    ...
    Write-Host "=== $Title Updater ===" -ForegroundColor Cyan
    Write-Host ""

    $IsInstalled    = & $TestInstalled $InstallDir
    $CurrentVersion = Get-CurrentVersion -Path $InstallDir

    if (-not $IsInstalled) {
        Write-Host "No binaries found in '$InstallDir'; running initial install." -ForegroundColor Yellow
        Write-Host ""
    }
